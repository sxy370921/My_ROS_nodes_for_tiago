...
def point_cloud_2_birdseye(points,
    ...
    z_points = points[:, 2] #z
    # 只取某个区域内的点云数据
    # 注意图像的原点在左上角，而点云投影以后的原点尽量放在图片下方居中，因此需要调节坐标系，靠side_range和height_range调节
    f_filt = np.logical_and((z_points > fwd_range[0]), (z_points < fwd_range[1]))
    s_filt = np.logical_and((x_points > side_range[0]), (x_points < side_range[1]))
    ...
    # 计算投影之后，点云对应二维像素的坐标.这里还起到了调节坐标系的作用
